...
/*         this.snakeGroup.children[0].children[1].position.x = 3;
    ...
    updateBody(x, y, z) {
        ...
        */
        ...
        if (this.snakeGroup.children.length > 1) {
            ...
            this.snakeGroup.children[1].visible = false; // trick GRAFICO
            ...
            for (var i = this.snakeGroup.children.length - 1; i > 1; i--) {
                ...
                this.snakeGroup.children[i].position.x = this.snakeGroup.children[i - 1].position.x - x * 1.1 + Math.cos(delta / 7 + i) / 8; // NEW 
                this.snakeGroup.children[i].position.y = this.snakeGroup.children[i - 1].position.y - y * 1.1 + Math.cos(delta / 7 + i) / 8; // NEW 
                this.snakeGroup.children[i].position.z = this.snakeGroup.children[i - 1].position.z - z * 1.1 + Math.cos(delta / 7 + i) / 8; // NEW 
    ...
    checkPosition(){
    	var originPoint = this.snakeGroup.children[0].position.clone();
        originPoint.z += 5;
        originPoint.y += 1.5;
        var destPoint = this.snakeGroup.children[0].position.clone();
        destPoint.z -= 1;
        destPoint.y += 1.5;
        
        var rc = new THREE.Raycaster( originPoint, destPoint );
        var collisionResults = rc.intersectObjects( game.scene.children );
